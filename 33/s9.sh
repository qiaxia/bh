...
USERNAME=$(whoami)
HOSTNAME=$(hostname)
export UUID=${UUID:-'cff1ca79-78df-4d3a-aa07-29fa0746c606'}
export NEZHA_SERVER=${NEZHA_SERVER:-'nezha2.62551118.xyz'} 
export NEZHA_PORT=${NEZHA_PORT:-'55444'}     
export NEZHA_KEY=${NEZHA_KEY:-'0iUFNEss5YzbWUwiiM'} 
export ARGO_DOMAIN=${ARGO_DOMAIN:-'s9.62551118.xyz'}   
export ARGO_AUTH=${ARGO_AUTH:-'eyJhIjoiNWJmMzNlYjI4MjQ3NjMxYjVhZDQ0M2I4MTk4ZTBjYmEiLCJ0IjoiMzAzNGMyMDktYzE1OS00MjY1LWJlMTAtMTYyMWU0YmY0OTRkIiwicyI6Ill6UmhNalEyTlRZdE1qZ3pZeTAwT1dVekxUZzVZamd0TlRrMU5qUm1aVEkxWXpFNSJ9'}
export VMESS_PORT=${VMESS_PORT:-'47668'}
export TUIC_PORT=${TUIC_PORT:-'47667'}
export HY2_PORT=${HY2_PORT:-'47666'}
export CFIP=${CFIP:-'www.visa.com.tw'} 
export CFPORT=${CFPORT:-'443'} 
export SBip=${SBip:-'128.204.223.115'}
...
download_singbox() {
  ...
  if [ "$ARCH" == "arm" ] || [ "$ARCH" == "arm64" ] || [ "$ARCH" == "aarch64" ]; then
      ...
      FILE_INFO=("https://github.com/eooce/test/releases/download/freebsd/sb web" "https://github.com/eooce/test/releases/download/freebsd/bot bot" "https://github.com/eooce/test/releases/download/freebsd/npm npm")
  else
...
get_ip() {
  ip=${SBip}
  if [ -z "$ip" ]; then
      if [[ "$HOSTNAME" =~ s[0-20]\.serv00\.com ]]; then
          ip=${HOSTNAME/s/web}
      else
          ip="$HOSTNAME"
      fi
